...
#tools used:Pycharm Community Edition
#took help from online resources
import time
...
def create(key,value,timeout=0):
    if key in d:
        print("error: this key",key," already exists") #error message1
    else:
        if(key.isalpha()):
...
def read(key):
    if key not in d:
        print("error: given key",key," does not exist in database. Please enter a valid key") #error message4
    else:
        b=d[key]
        if b[1]!=0:
...
def modify(key,value):
    ...
    if b[1]!=0:
        if time.time()<b[1]:
            if key not in d:
                print("error: given key",key," does not exist in database. Please enter a valid key") #error message6
            else:
                l=[]
                l.append(value)
                l.append(b[1])
